...
function Kids() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch("http://localhost:3005/productos/kids"); // Cambia esta URL por tu endpoint
  ...
  if (loading) {
    return <div className="loading">Cargando productos...</div>; // Muestra un mensaje mientras carga
  }
    ...
    <div className="kids">
      ...
      <header className="header">
        <div className="header-title">
          <h1>Catálogo de Ropa para Niño/a ({products.length})</h1>
          <p>Explora nuestra colección de ropa y accesorios para niños y niñas</p>
        </div>
      </header>
